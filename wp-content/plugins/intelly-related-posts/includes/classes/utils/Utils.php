...
class IRP_Utils {
    ...
    function remoteGet($uri, $options) {
        ...
        $irp->Log->debug('REMOTEGET: URI=%s', $uri);
        $response=wp_remote_get($uri, $args);
        if (!is_wp_error($response)) {
            // decode the license data
            $body=wp_remote_retrieve_body($response);
            if($body!==FALSE && $body!='') {
                $irp->Log->debug('REMOTEGET: RETRIEVEBODY=%s', $body);
                $result=json_decode($body);
                if(!$result) {
                    $irp->Log->error('REMOTEGET: UNDECODABLE BODY');
                    if(strpos($body, 'debug')!==FALSE || strpos($body, 'fatal')!==FALSE || strpos($body, 'warning')!==FALSE || strpos($body, 'error')!==FALSE) {
                        $irp->Options->pushErrorMessage('XdebugException');
                    }
                }
            } else {
                $irp->Log->debug('REMOTEGET: RETRIEVEBODY ERROR');
                $result=FALSE;
            }
        } else {
            $irp->Log->debug('REMOTEGET: RESULT=ERROR');
        }
        if($result===FALSE) {
            $irp->Log->debug('file_get_contents: URI=%s', $uri);
            $body=file_get_contents($uri);
            $irp->Log->debug('file_get_contents: RETRIEVEBODY=%s', $body);
            if($body!==FALSE && $body!='') {
                $irp->Log->debug('REMOTEGET: RETRIEVEBODY=%s', $body);
                $result=json_decode($body);
                if(!$result) {
