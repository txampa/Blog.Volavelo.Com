...
$language = get_locale();
if ($language == 'es_ES') {
previous_post_link('%link', __('Artículo anterior', 'mh-magazine-lite'));
}
else {
previous_post_link('%link', __('Article précédent', 'mh-magazine-lite'));
}
				...
				if ($next || $attachment) {
					echo '<div class="mh-post-nav-next mh-post-nav">' . "\n";
						if ($attachment && wp_attachment_is_image($post->id)) {
							...
							$language = get_locale();
if ($language == 'es_ES') {
previous_post_link('%link', __('Artículo siguiente', 'mh-magazine-lite'));
}
else {
