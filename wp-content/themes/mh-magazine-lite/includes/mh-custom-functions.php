				...
				if ($previous || $attachment) {
					echo '<div class="mh-post-nav-prev mh-post-nav">' . "\n";
						if ($attachment) {
							...
							$language = get_locale();
        							if ($language == 'es_ES') {
														previous_post_link('%link', __('Art&Iacute;culo Anterior', 'mh-magazine-lite'));
											}
											else {
														previous_post_link('%link', __('Article pr&Egrave;c&Egrave;dent', 'mh-magazine-lite'));
											}
				...
				if ($next || $attachment) {
					echo '<div class="mh-post-nav-next mh-post-nav">' . "\n";
						if ($attachment && wp_attachment_is_image($post->id)) {
							...
							$language = get_locale();
        							if ($language == 'es_ES') {
														previous_post_link('%link', __('Art&Iacute;culo Siguiente', 'mh-magazine-lite'));
											}
											else {
														previous_post_link('%link', __('Article suivant', 'mh-magazine-lite'));
											}
						}
					echo '</div>' . "\n";
				}
